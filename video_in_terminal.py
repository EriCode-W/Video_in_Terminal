...
class V2Char(CharFrame):
    ...
    def genCharVideo(self, filepath):
        ...
        self.timeInterval = round(1 / cap.get(5), 3) # cap.get(5) framerate
        nf = int(cap.get(7)) # cap.get(7) framenum
        print('Generate char video, please wait...')
        for i in pyprind.prog_bar(range(nf)): # 进度条
            # 转换颜色空间，第二个参数是转换类型，cv2.COLOR_BGR2GRAY表示从BGR↔Gray
            try:
                rawFrame = cv2.cvtColor(cap.read()[1], cv2.COLOR_BGR2GRAY) # ret, frame = cap.read()
